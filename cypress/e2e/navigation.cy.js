// Navigation Tests - Recipe Book
describe('Navigation - Recipe Book', () => {
  beforeEach(() => {
    cy.visit('/');
  });
  ...
  it('loads the homepage successfully', () => {
    ...
    cy.get('.nav-brand').should('contain', 'Recipe Book');
  });
  ...
  it('navigates to recipes list page', () => {
  ...
  it('cancel button returns to recipes list', () => {
    cy.get('.nav-link[data-page="add"]').click();
    cy.get('#btn-cancel').click();
    cy.get('#page-recipes').should('not.have.class', 'hidden');
  });

  it('shows add new button on recipes list page', () => {
    cy.get('.nav-link[data-page="recipes"]').click();
  ...
  it('api health endpoint returns correct project name', () => {
    cy.request('/api/health').its('body.project').should('eq', 'Recipe Book');
  });
});
