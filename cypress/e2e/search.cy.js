// Search and Filter Tests - Recipe Book
describe('Search and Filter - Recipe Book', () => {
  beforeEach(() => {
    cy.visit('/');
  ...
  it('searching for existing item title shows result', () => {
    cy.get('#recipes-list .item-card').first().find('.item-title').invoke('text').then(title => {
  ...
  it('searching for non-existent term shows empty state', () => {
    ...
    cy.get('#recipes-list').should('contain', 'No items found');
  });
  ...
  it('clearing search restores full list', () => {
    cy.get('#recipes-list .item-card').then($cards => {
      ...
      cy.get('#recipes-list .item-card').should('have.length', total);
    });
  });
  ...
  it('filter by Breakfast category works', () => {
    cy.get('#filter-category').select('Breakfast');
  ...
  it('api search endpoint filters results', () => {
    cy.request('/api/recipes?search=test').its('body').should('be.an', 'array');
  });

  it('api category filter endpoint works', () => {
    cy.request('/api/recipes?category=Breakfast').its('body').should('be.an', 'array');
  });
  ...
  it('api returns all items when no filter applied', () => {
    cy.request('/api/recipes').its('body').should('have.length.gte', 1);
  });
