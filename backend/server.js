...
app.get('/api/recipes', (req, res) => {
  ...
  if (shouldBeFlaky(FLAKY_CONFIG.slowProbability)) {
    ...
    console.log(`[FLAKY] Slow GET /api/recipes +${delay}ms`);
...
app.post('/api/recipes', (req, res) => {
  if (shouldBeFlaky(FLAKY_CONFIG.errorProbability)) {
    console.log(`[FLAKY] 500 error on POST /api/recipes`);
...
app.listen(PORT, () => console.log('Recipe Book running on http://localhost:3002 [FLAKY v2]'));
